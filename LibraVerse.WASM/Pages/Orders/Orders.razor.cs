...
public partial class Orders
{
    ...
    private async Task LoadUsers()
    {
        try
        {
            _users = (await ApiClient.GetAllUsersListAsync(null, true, null)).ToList();
        }
        catch (Exception ex)
        {
    ...
    private async Task HandleUserChange(ChangeEventArgs e)
    {
        if (e.Value != null)
        {
    ...
    private async Task LoadOrders()
    {
        ...
        try
        {
            Guid? _selectedUserId = string.IsNullOrEmpty(_selectedUser) ? null : Guid.Parse(_selectedUser);

            orders = await ApiClient.GetAllOrdersAsync(currentPage, pageSize, _orderStatus, _selectedUserId);
        }
        catch (Exception ex)
        {
    ...
    private async Task CompleteOrder(OrderDto order, string claimCode)
    {
        try
        {
            ...
            await ApiClient.UpdateOrderAsync(order.Id, orderDetails);
